...
class RealTimeScanner:
    ...
    async def start(self):
        ...
        This method runs a single scan and then exits. For continuous scanning,
        use the scheduler.
        """
        try:
            ...
            logger.info(f"Scan completed: detected {scan_results['total_items_count']} items")
            return scan_results
        
        except Exception as e:
            logger.error(f"Error in real-time scanner: {e}")
            log_action('real_time_scanner', 'start', f"Error: {e}", status='error')
