...
export const usePrompts = () => {
  ...
  // Run prompt
  const runPrompt = useCallback((prompt: string, model: string, useContext: boolean = false, contextQuery: string = '', contextLimit: number = 5) => {
    return runMutation.mutateAsync({ prompt, model, use_context: useContext, context_query: contextQuery, context_limit: contextLimit }, {
      onSuccess: (result) => {
